...
class AppData extends AppDataBase {
    ...
    *__enumerateDbPaths() {
        yield g_dbRoot + "fe-the-sacred-stones.sqlite3?20250629";
    }

    __createQueryPostProcess(query) {
        if (this.removesDuplicates) {
            query += " GROUP BY CASE WHEN name1 < name2 THEN name1 ELSE name2 END, CASE WHEN name1 < name2 THEN name2 ELSE name1 END";
        }
        return query;
    }
}
